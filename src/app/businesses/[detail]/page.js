...
async function getData(id, user_id, token) {
  ...
  if (
    resDetails.status === 401 ||
    resEvent.status === 401 ||
    resVoopons.status === 401 ||
    resAllPhotos.status === 401 ||
    resRating.status === 401
  ) {
  ...
  // Check for API response code 401
  if (
    resultDetails?.code == 401 ||
    resultEvent?.code == 401 ||
    reslutVoopons?.code == 401 ||
    resultAllPhotos?.code == 401 ||
    resBusinessRating?.code == 401
  ) {
    const error = new Error("Unauthorized");
    error.status = 401;
    throw error;
...
const Detail = () => {
  ...
  const handleUnauthorized = () => {
    ...
    router.push(`/login?lastPath=${encodeURIComponent(tempPathName)}`);
  };
  ...
  const tabs = {
    ...
    voopans: data?.VooponsList,
    photos: data?.AllPhotosList,
